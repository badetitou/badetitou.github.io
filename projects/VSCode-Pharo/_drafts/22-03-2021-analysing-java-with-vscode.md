...
- Create a new code cell with the following piece of code to download the last VerveineJ version and extract it
  ...
  ```
- Then, we will ask `MooseEasyFamixMaker` to parse the Java project using VerveineJ, and create an `.mse` file for us. Again, in another cell, execute the following snippet of code.
  ```st
...
Congrats! you have set up everything. Now it's time to analyze the project :rocket:
...
It is possible to use pre-built visualisation coming from Moose, such as the System Nesting map:
