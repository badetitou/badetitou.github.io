...
pas 1
d (0.15) e (0.1)
Figure 2 – Arbre des Probabilités
...
la structure de données schématisée dans la table 5 sous forme de tableau (fg signifie fils gauche tandis
que fd signifie fils droit).
...
1. Construction de l’arbre. L’objet de cet algorithme est de construire un arbre binaire complet.
...
2) permet de construire les arcs reliant le nouveau noeud virtuel à ses deux fils.
3. Calcul du code. On va calculer le code d’un chiffre donné en parcourant l’arbre. Nous utiliserons le
...
4. l’arbre le plus déséquilibré aura une profondeur de 255 donc la taille maxi de code sera de 255 bits tandis
